...
$events = [];
try {
    $events = $pdo->query("SELECT * FROM event ORDER BY position ASC LIMIT 3")->fetchAll();
} catch (Exception $e) {
}

$testimonials = [];
try {
    $testimonials = $pdo->query("SELECT * FROM testimonial WHERE status='approved' ORDER BY position ASC LIMIT 5")->fetchAll();
} catch (Exception $e) {
}

$db_uni = [];
try {
    $db_uni = $pdo->query("SELECT * FROM university ORDER BY position ASC")->fetchAll();
} catch (Exception $e) {
}
...
<section class="relative bg-white pt-24 pb-20 lg:pt-32 lg:pb-28 overflow-hidden">
    ...
    <div
        class="absolute top-0 right-0 w-[500px] h-[500px] bg-primary rounded-full filter blur-[120px] opacity-70 translate-x-1/3 -translate-y-1/3">
    </div>
    <div
        class="absolute bottom-0 left-0 w-[600px] h-[600px] bg-orange-50 rounded-full filter blur-[150px] opacity-60 -translate-x-1/4 translate-y-1/4">
    </div>
    ...
    <div class="container mx-auto px-4 relative z-10 text-center mt-8">
        <div class="max-w-4xl mx-auto">
            <?php if ($hero_slide): ?>
                <h1
                    class="text-4xl sm:text-5xl lg:text-7xl font-extrabold leading-tight tracking-tight text-slate-800 mb-6 animate-on-scroll">
                    <?php echo htmlspecialchars($hero_slide['title']); ?>
                </h1>
                <p
                    class="text-lg sm:text-xl text-slate-500 mb-10 font-light max-w-2xl mx-auto leading-relaxed animate-on-scroll delay-100">
                ...
                <div class="flex flex-wrap justify-center gap-4 animate-on-scroll delay-200">
                    ...
                    <?php if (!empty($hero_slide['button2_text'])): ?>
                        <a href="<?php echo htmlspecialchars($hero_slide['button2_link'] ?: '#'); ?>"
                            class="bg-white border border-slate-200 hover:border-secondary hover:text-secondary text-slate-600 px-8 py-3.5 rounded-full font-bold transition transform hover:-translate-y-1 flex items-center gap-2 shadow-sm">
                            <?php echo htmlspecialchars($hero_slide['button2_text']); ?> <i class="ph ph-arrow-right"></i>
                        </a>
                    <?php endif; ?>
                </div>
            <?php else: ?>
                <h1 class="text-5xl lg:text-7xl font-extrabold leading-tight text-slate-800 mb-6">Empower Your <span
                        class="text-secondary">Future</span></h1>
...
<?php if (!empty($partners)): ?>
    <section class="py-12 bg-slate-50 border-y border-slate-100 overflow-hidden">
        <div class="container mx-auto px-4 mb-8 text-center">
            <p class="text-sm font-bold text-slate-400 uppercase tracking-widest">Our Global Industry Partners</p>
        </div>
        <div class="logo-marquee-container group">
...
<section class="py-20 bg-primary relative">
    <div class="container mx-auto px-4">
        <div class="text-center max-w-3xl mx-auto mb-16 animate-on-scroll">
            ...
            <h2 class="text-3xl md:text-4xl font-bold text-dark">Services We <span class="text-secondary">Provide</span>
            </h2>
            <p class="text-slate-500 mt-4 leading-relaxed">
                Comprehensive support tailored to ensure your international education journey is seamless and
                successful.
            </p>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            <?php foreach ($services as $index => $service): ?>
                <div class="bg-white rounded-2xl p-8 shadow-sm hover:shadow-xl transition duration-300 border border-slate-100 group animate-on-scroll"
                    style="animation-delay: <?php echo ($index * 100); ?>ms;">

                    <div
                        class="w-16 h-16 rounded-2xl flex items-center justify-center text-3xl mb-6 shadow-sm group-hover:scale-110 transition duration-300 transform 
                    ...
                    <h3 class="text-xl font-bold text-dark mb-3 group-hover:text-secondary transition">
                        <?php echo htmlspecialchars($service['title']); ?></h3>
        ...
        <div class="container mx-auto px-4">
            <div class="flex flex-col md:flex-row justify-between items-end mb-12 animate-on-scroll">
                <div class="max-w-xl">
                    ...
                    <h2 class="text-3xl md:text-4xl font-bold text-dark">Top Study <span
                            class="text-secondary">Destinations</span></h2>
                ...
                <?php foreach (array_slice($countries, 0, 4) as $index => $country): ?>
                    <a href="country-view.php?id=<?php echo urlencode($country['id']); ?>"
                        ...
                        <?php if ($country['country_image']): ?>
                            <img src="uploads/countries/<?php echo htmlspecialchars($country['country_image']); ?>"
                                class="w-full h-full object-cover group-hover:scale-110 group-hover:rotate-1 transition duration-700 ease-out">
                        <?php else: ?>
                        ...
                        <!-- Gradient Overlay -->
                        <div
                            class="absolute inset-0 bg-gradient-to-t from-dark/95 via-dark/50 to-transparent opacity-95 transition duration-500">
                        </div>
                        ...
                        <div class="absolute inset-0 p-6 flex flex-col justify-end">
                            <div class="transition duration-500 ease-out pb-2">
                                <h3 class="text-2xl font-bold text-white mb-2">
                                    <?php echo htmlspecialchars($country['country_name']); ?></h3>

                                <p class="text-slate-300 text-sm line-clamp-2 mb-5 leading-relaxed font-light">Explore
                                    world-class universities and vibrant student life in
                                    <?php echo htmlspecialchars($country['country_name']); ?>.</p>
                                <span
                                    class="bg-secondary text-white px-5 py-2.5 rounded-full text-xs font-bold inline-flex items-center gap-2 shadow-lg shadow-orange-500/20 w-fit">
...
<?php if (!empty($countries) && !empty($db_uni)): ?>
    <section class="py-24 bg-slate-50 relative border-t border-slate-100 overflow-hidden">
        <div class="container mx-auto px-4">
            <div class="text-center max-w-2xl mx-auto mb-12 animate-on-scroll">
                <h4 class="text-secondary font-bold tracking-wider uppercase text-sm mb-2">Prestigious Institutions</h4>
                <h2 class="text-3xl md:text-4xl font-bold text-dark">Top Universities by <span class="text-secondary">Country</span></h2>
            </div>
            
            <!-- Tabs Navigation -->
            <div class="flex flex-wrap justify-center gap-3 mb-10 animate-on-scroll delay-100">
                <?php foreach($countries as $index => $country): ?>
                    <button class="uni-tab-btn px-6 py-2.5 rounded-full font-bold text-sm transition-all duration-300 border-2 <?php echo $index === 0 ? 'bg-secondary text-white border-secondary shadow-lg shadow-orange-500/30 active-tab' : 'bg-transparent border-slate-200 text-slate-500 hover:border-secondary hover:text-secondary inactive-tab'; ?>" data-target="tab-<?php echo preg_replace('/[^A-Za-z0-9\-]/', '', str_replace(' ', '-', strtolower($country['country_name']))); ?>">
            ...
            <div class="relative min-h-[250px]">
                <?php foreach($countries as $index => $country): 
                    $country_unis = array_filter($db_uni, function($u) use ($country) {
                        return isset($u['country']) && strtolower(trim($u['country'])) === strtolower(trim($country['country_name']));
                    });
                    $country_unis = array_slice($country_unis, 0, 10); // Limit to 10
                ?>
                    <div id="tab-<?php echo preg_replace('/[^A-Za-z0-9\-]/', '', str_replace(' ', '-', strtolower($country['country_name']))); ?>" class="uni-tab-content transition-all duration-500 <?php echo $index === 0 ? 'opacity-100 relative z-10' : 'opacity-0 absolute inset-0 -z-10 pointer-events-none hidden'; ?>">
                            ...
                            <?php if(!empty($country_unis)): ?>
                                <?php foreach($country_unis as $uni): ?>
                                    <a href="university-view.php?id=<?php echo $uni['id']; ?>" title="<?php echo htmlspecialchars($uni['university_name']); ?>" class="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 hover:shadow-xl hover:border-secondary transition duration-300 flex items-center justify-center aspect-[4/3] group relative overflow-hidden">
                                        ...
                                        <?php else: ?>
                                            <i class="ph ph-graduation-cap text-4xl text-slate-300 group-hover:text-secondary transition transform group-hover:scale-110"></i>
                                            <span class="absolute bottom-2 text-[10px] text-slate-400 font-bold uppercase text-center w-full truncate px-2"><?php echo htmlspecialchars($uni['university_name']); ?></span>
                                        <?php endif; ?>
                                    </a>
                                <?php endforeach; ?>
                            <?php else: ?>
                                <div class="col-span-full py-12 text-center text-slate-400 bg-white rounded-3xl border border-slate-100 shadow-sm">
                                    <div class="w-16 h-16 bg-slate-50 rounded-full flex items-center justify-center mx-auto mb-3 text-2xl"><i class="ph ph-buildings"></i></div>
                                    <p class="font-medium">Expanding our partnerships in <?php echo htmlspecialchars($country['country_name']); ?>.</p>
                                </div>
                            <?php endif; ?>
                        </div>
                    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            const tabBtns = document.querySelectorAll('.uni-tab-btn');
            const tabContents = document.querySelectorAll('.uni-tab-content');

            tabBtns.forEach(btn => {
                btn.addEventListener('click', () => {
                    // Reset all buttons
                    tabBtns.forEach(b => {
                        b.classList.remove('bg-secondary', 'text-white', 'border-secondary', 'shadow-lg', 'shadow-orange-500/30', 'active-tab');
                        b.classList.add('bg-transparent', 'border-slate-200', 'text-slate-500', 'inactive-tab');
                    });
                    
                    // Activate this button
                    btn.classList.add('bg-secondary', 'text-white', 'border-secondary', 'shadow-lg', 'shadow-orange-500/30', 'active-tab');
                    btn.classList.remove('bg-transparent', 'border-slate-200', 'text-slate-500', 'inactive-tab');

                    // Reset all contents
                    tabContents.forEach(content => {
                        content.classList.remove('opacity-100', 'relative', 'z-10');
                        content.classList.add('opacity-0', 'absolute', 'inset-0', '-z-10', 'pointer-events-none', 'hidden');
                    });

                    // Activate target content
                    const target = document.getElementById(btn.getAttribute('data-target'));
                    target.classList.remove('opacity-0', 'absolute', 'inset-0', '-z-10', 'pointer-events-none', 'hidden');
                    // slight delay to allow display:block to apply before animating opacity
                    setTimeout(() => {
                        target.classList.add('opacity-100', 'relative', 'z-10');
                    }, 50);
                });
            });
        });
    ...
    <div class="container mx-auto px-4 flex flex-col lg:flex-row items-center gap-16">
        <div class="lg:w-1/2 relative animate-on-scroll">
            <div class="relative z-10 rounded-3xl overflow-hidden shadow-lg border border-slate-100">
                <img src="assets/images/student-studying.png" alt="Students studying"
        ...
        <div class="lg:w-1/2 animate-on-scroll">
            ...
            <h2 class="text-3xl md:text-4xl font-bold text-slate-800 mb-6 leading-tight">Your Trusted Guide to <br><span
                    class="text-secondary">International Education</span></h2>
            <p class="text-slate-500 mb-6 leading-relaxed">
                At Unilink Global Solution, we believe that education knows no borders. We are dedicated to helping
                ambitious students find the perfect university matching their academic background and future career
                goals.
            </p>
            <ul class="space-y-4 mb-8">
                <li class="flex items-center gap-3 text-slate-700 font-medium"><i
                        class="ph-fill ph-check-circle text-secondary text-xl"></i> Authorized representation of top
                    universities.</li>
                <li class="flex items-center gap-3 text-slate-700 font-medium"><i
                        class="ph-fill ph-check-circle text-secondary text-xl"></i> Transparent and ethical counseling.
                </li>
                <li class="flex items-center gap-3 text-slate-700 font-medium"><i
                        class="ph-fill ph-check-circle text-secondary text-xl"></i> High visa success rate.</li>
                <li class="flex items-center gap-3 text-slate-700 font-medium"><i
...
<section class="py-24 bg-primary relative border-y border-slate-100">
    <div class="container mx-auto px-4 text-center max-w-3xl mb-16 animate-on-scroll">
        ...
        <h2 class="text-3xl md:text-4xl font-bold text-dark">What Our Students <span class="text-secondary">Say</span>
        </h2>
    </div>
                ...
                <div class="swiper-wrapper">
                    <?php foreach ($testimonials as $testi): ?>
                        <div class="swiper-slide h-auto">
                            <div
                                class="bg-white p-8 rounded-2xl border border-slate-100 h-full flex flex-col items-center text-center shadow-sm relative">
                                <i class="fa-solid fa-quote-left text-4xl text-slate-100 absolute top-6 left-6"></i>
                                <p class="text-slate-600 italic mb-8 relative z-10">
                                    "<?php echo htmlspecialchars($testi['message']); ?>"</p>
                                <div class="mt-auto">
                                    <?php if ($testi['image']): ?>
                                        <img src="uploads/testimonials/<?php echo htmlspecialchars($testi['image']); ?>"
                                            class="w-16 h-16 rounded-full object-cover mx-auto mb-3 border-2 border-primary">
                                    <?php else: ?>
                                        <div
                                            class="w-16 h-16 rounded-full bg-secondary text-white flex items-center justify-center text-2xl mx-auto mb-3 font-bold">
                                            <?php echo substr($testi['name'], 0, 1); ?></div>
                                    <?php endif; ?>
                                    <h4 class="font-bold text-dark text-lg"><?php echo htmlspecialchars($testi['name']); ?></h4>
                                    <p class="text-secondary text-sm font-semibold capitalize">
                                        <?php echo htmlspecialchars($testi['role']); ?></p>
                                </div>
                            </div>
                        </div>
                    <?php endforeach; ?>
        ...
        <div class="container mx-auto px-4">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <?php foreach ($events as $index => $event): ?>
                    <div class="flex flex-col bg-primary rounded-2xl overflow-hidden shadow-sm hover:shadow-lg transition group animate-on-scroll"
                        style="animation-delay: <?php echo ($index * 100); ?>ms;">
                        <div class="h-48 overflow-hidden relative">
                            ...
                            <!-- Event Date Badge -->
                            <div
                                class="absolute top-4 left-4 bg-secondary text-white font-bold py-2 px-4 rounded-xl shadow-md text-center">
                                <span class="block text-2xl"><?php echo date('d', strtotime($event['date_time'])); ?></span>
                                <span
                                    class="block text-xs uppercase tracking-wider"><?php echo date('M', strtotime($event['date_time'])); ?></span>
                            </div>
                        </div>
    ...
    <div class="container mx-auto px-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            <?php foreach ($blogs as $index => $blog): ?>
                <div class="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition duration-300 border border-slate-100 group animate-on-scroll"
                    style="animation-delay: <?php echo ($index * 100); ?>ms;">
                    <div class="h-56 overflow-hidden relative">
                        <?php if ($blog['image']): ?>
                            <img src="uploads/blogs/<?php echo htmlspecialchars($blog['image']); ?>"
                                class="w-full h-full object-cover group-hover:scale-105 transition duration-500">
                        <?php else: ?>
                            <div class="w-full h-full bg-slate-100 flex items-center justify-center text-slate-300"><i
                                    class="ph ph-image text-4xl"></i></div>
                        <?php endif; ?>
                        <!-- Date Badge -->
                        <div
                            class="absolute top-4 right-4 bg-white/90 backdrop-blur text-secondary font-bold py-1 px-3 rounded-lg shadow-sm text-sm text-center">
                            <span class="block text-lg"><?php echo date('d', strtotime($blog['created_at'])); ?></span>
                            <span
                                class="block text-xs uppercase"><?php echo date('M', strtotime($blog['created_at'])); ?></span>
                        </div>
                    </div>
                    <div class="p-6">
                        ...
                        <div class="mt-4 border-t border-slate-100 pt-4 flex items-center justify-between">
                            <div class="flex items-center gap-3 text-slate-500 text-xs font-semibold">
                                <span class="flex items-center gap-1" title="Likes"><i
                                        class="ph-fill ph-heart text-secondary"></i> <?php echo $blog['likes']; ?></span>
                                <span class="flex items-center gap-1" title="Comments"><i
                                        class="ph-fill ph-chat-circle text-primary"></i>
                                    <?php echo $blog['comment_count']; ?></span>
                                <span class="flex items-center gap-1" title="Shares"><i
